...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // === 04b: THEME APPLICATION START ===
    function applyThemeColors(bgColor, textColor) {
        document.documentElement.style.setProperty('--live-page-bg', bgColor);
        document.documentElement.style.setProperty('--live-page-text', textColor);
        // Calculate border color dynamically based on background for better visibility
        const [r, g, b] = parseRgbString(bgColor);
        const brightness = (r * 299 + g * 587 + b * 114) / 1000; // Simple brightness calc
        const borderColor = brightness < 128 ? 'rgba(255, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.2)'; // Light border on dark, dark on light
        document.documentElement.style.setProperty('--live-split-border', borderColor);
        console.log(`Theme applied: BG=${bgColor}, Text=${textColor}, Border=${borderColor}`);
    }
    // Load and apply theme on initial load
    const initialBgColor = loadThemeBgColor();
    const initialTextColor = loadThemeTextColor();
    applyThemeColors(initialBgColor, initialTextColor);
    // === 04b: THEME APPLICATION END ===
    ...
    function checkAndHandleTableBreak() { const activePlayersCount = state.live.players.length; const playersPerTable = state.config.playersPerTable; const currentTablesSet = new Set(state.live.players.map(p => p.table)); const currentTableCount = currentTablesSet.size; const targetTables = Math.ceil(activePlayersCount / playersPerTable); const finalTableSize = playersPerTable; let actionTaken = false; let redrawPerformed = false; if (activePlayersCount <= finalTableSize && currentTableCount > 1) { const targetFinalTableNum = 1; logActivity(state.live.activityLog, `Finalebord! Flytter alle til Bord ${targetFinalTableNum} og trekker seter...`); alert(`Finalebord! Flytter alle til Bord ${targetFinalTableNum} og trekker seter.`); state.live.players.forEach(p => p.table = targetFinalTableNum); reassignAllSeats(targetFinalTableNum); actionTaken = true; redrawPerformed = true; } else if (currentTableCount > targetTables && currentTableCount > 1) { const tables = {}; state.live.players.forEach(p => { tables[p.table] = (tables[p.table] || 0) + 1; }); const sortedTables = Object.entries(tables).map(([num, count]) => ({ tableNum: parseInt(num), count: count })).sort((a, b) => a.count - b.count); const tableToBreak = sortedTables[0].tableNum; const breakMsg = `Slår sammen bord! Flytter spillere fra bord ${tableToBreak}.`; logActivity(state.live.activityLog, breakMsg); alert(breakMsg); const playersToMove = state.live.players.filter(p => p.table === tableToBreak); playersToMove.forEach(player => { const oldTable = player.table; const oldSeat = player.seat; player.table = 0; assignTableSeat(player, tableToBreak); logActivity(state.live.activityLog, ` -> Flyttet ${player.name} (B${oldTable}S${oldSeat}) til B${player.table}S${player.seat}.`); }); state.live.players.sort((a, b) => a.table === b.table ? a.seat - b.seat : a.table - b.table); actionTaken = true; } if (actionTaken) { updateUI(); saveTournamentState(currentTournamentId, state); } return actionTaken; }
    function balanceTables() { if (state.live.status === 'finished') return false; const tables = {}; state.live.players.forEach(p => { tables[p.table] = (tables[p.table] || 0) + 1; }); const tableCounts = Object.entries(tables).map(([num, count]) => ({ tableNum: parseInt(num), count: count })).filter(tc => tc.count > 0); if (tableCounts.length < 2) { tableBalanceInfo.classList.add('hidden'); return false; } tableCounts.sort((a, b) => a.count - b.count); const minTable = tableCounts[0]; const maxTable = tableCounts[tableCounts.length - 1]; if (maxTable.count - minTable.count < 2) { tableBalanceInfo.classList.add('hidden'); return false; } tableBalanceInfo.classList.remove('hidden'); console.log(`Balancing: Max T${maxTable.tableNum}(${maxTable.count}), Min T${minTable.tableNum}(${minTable.count})`); const playersOnMaxTable = state.live.players.filter(p => p.table === maxTable.tableNum); if (playersOnMaxTable.length === 0) { console.error("Balancing Error!"); return false; } const playerToMove = playersOnMaxTable[Math.floor(Math.random() * playersOnMaxTable.length)]; const occupiedSeatsMin = state.live.players.filter(p => p.table === minTable.tableNum).map(p => p.seat); let newSeat = 1; while (occupiedSeatsMin.includes(newSeat)) { newSeat++; } if (newSeat > state.config.playersPerTable) { console.error(`Balancing Error: No seat on table ${minTable.tableNum}.`); alert(`Balanseringsfeil bord ${minTable.tableNum}.`); return false; } const oldTable = playerToMove.table; const oldSeat = playerToMove.seat; const message = `Balansering: ${playerToMove.name} flyttes fra B${oldTable} S${oldSeat} til B${minTable.tableNum} S${newSeat}.`; playerToMove.table = minTable.tableNum; playerToMove.seat = newSeat; logActivity(state.live.activityLog, message); state.live.players.sort((a, b) => a.table === b.table ? a.seat - b.seat : a.table - b.table); saveTournamentState(currentTournamentId, state); updateUI(); alert(message); console.log("Balancing move applied."); return true; }
    ...
    function displayPrizes() {
        if (!prizeDisplayLive) return;
        const prizeData = calculatePrizes();
        const potDisplayElement = prizeDisplayLive.querySelector('#total-pot'); // Get span inside h3
        if(potDisplayElement) potDisplayElement.textContent = state.live.totalPot.toLocaleString('nb-NO');
        else prizeDisplayLive.querySelector('h3').textContent = `Premiefordeling (Totalpott: ${state.live.totalPot.toLocaleString('nb-NO')} kr)`;

        const existingOl = prizeDisplayLive.querySelector('ol'); if (existingOl) existingOl.remove();
        const existingP = prizeDisplayLive.querySelector('p'); if (existingP) existingP.remove();

        if (prizeData.length > 0) { const ol = document.createElement('ol'); prizeData.forEach(p => { const li = document.createElement('li'); li.textContent = `${p.place}. Plass: ${p.amount.toLocaleString('nb-NO')} kr (${p.percentage}%)`; ol.appendChild(li); }); prizeDisplayLive.appendChild(ol); prizeDisplayLive.classList.remove('hidden'); }
        else { const p = document.createElement('p'); const potForPrizes = state.live.totalPot - (state.config.type === 'knockout' ? state.live.totalEntries * (state.config.bountyAmount || 0) : 0); if (potForPrizes > 0 && (!state.config.prizeDistribution || state.config.prizeDistribution.length === 0 || state.config.prizeDistribution.length !== state.config.paidPlaces)) { p.textContent = 'Premiefordeling ikke (korrekt) definert.'; } else if (potForPrizes <= 0) { p.textContent = 'Ingen premiepott å fordele ennå.'; } else { p.textContent = 'Beregner...'; } prizeDisplayLive.appendChild(p); prizeDisplayLive.classList.add('hidden'); }
    }
    function updateUI() {
        const elementsToCheck = { nameDisplay, currentTimeDisplay, timerDisplay, currentLevelDisplay, nextBlindsDisplay, blindsDisplay, breakInfo, playersRemainingDisplay, totalEntriesDisplay, averageStackDisplay, /* totalPotDisplay removed from check, shown in prize header */ lateRegStatusDisplay, lateRegButton, startPauseButton, prevLevelButton, nextLevelButton, adjustTimeMinusButton, adjustTimePlusButton, btnEditSettings, endTournamentButton, prizeDisplayLive, /* prizeSummarySpan removed */ playerListUl, eliminatedPlayerListUl, activePlayerCountSpan, eliminatedPlayerCountSpan, tableBalanceInfo, activityLogUl }; let missingElement = null; for (const key in elementsToCheck) { if (!elementsToCheck[key]) { missingElement = key; break; } } if (missingElement) { console.error(`CRITICAL ERROR: UI element "${missingElement}" missing.`); if(timerInterval) clearInterval(timerInterval); if(realTimeInterval) clearInterval(realTimeInterval); document.body.innerHTML = `<h1 style="color:red; text-align:center; margin-top: 50px;">UI Feil! Element "${missingElement}" mangler.</h1>`; return; }
        ...
        let nextBlindLevel = null; for(let i = cLI + 1; i < state.config.blindLevels.length; i++) { nextBlindLevel = state.config.blindLevels[i]; break; } // Find next level object

        currentLevelDisplay.textContent = `(Nivå ${cL ? cL.level : 'N/A'})`;
        nextBlindsDisplay.textContent = formatNextBlindsText(nextBlindLevel); // Always show next level blinds

        if (state.live.isOnBreak) { timerDisplay.textContent = formatTime(state.live.timeRemainingInBreak); blindsDisplay.innerHTML = ''; blindsDisplay.classList.add('hidden'); breakInfo.classList.remove('hidden'); breakInfo.textContent = `PAUSE`; }
        ...
        const apc = state.live.players.length; playersRemainingDisplay.textContent = apc; totalEntriesDisplay.textContent = state.live.totalEntries; averageStackDisplay.textContent = calculateAverageStack().toLocaleString('nb-NO');
    ...
    function openEditModal() {
        ...
        // Store original theme for potential cancel
    ...
    function updateColorPreviews() {
        const bgR = bgRedSlider.value; const bgG = bgGreenSlider.value; const bgB = bgBlueSlider.value;
        const textR = textRedSlider.value; const textG = textGreenSlider.value; const textB = textBlueSlider.value;
        ...
        if(textColorPreview) {
            textColorPreview.style.backgroundColor = currentBgColor; // Show text against current background
            textColorPreview.querySelector('span').style.color = currentTextColor; // Set text color
        }
        // Apply live preview to the actual page
        applyThemeColors(currentBgColor, currentTextColor);
    }
    function handleThemeSliderInput(e) {
        const type = e.target.id.includes('bg') ? 'bg' : 'text';
        const color = e.target.id.includes('Red') ? 'Red' : e.target.id.includes('Green') ? 'Green' : 'Blue';
        const inputElement = document.getElementById(`${type}${color}Input`);
        if (inputElement) inputElement.value = e.target.value;
        updateColorPreviews();
    }
    function handleThemeNumberInput(e) {
         const type = e.target.id.includes('bg') ? 'bg' : 'text';
        const color = e.target.id.includes('Red') ? 'Red' : e.target.id.includes('Green') ? 'Green' : 'Blue';
        const sliderElement = document.getElementById(`${type}${color}Slider`);
        let value = parseInt(e.target.value);
        if (isNaN(value)) value = 0;
        value = Math.max(0, Math.min(255, value)); // Clamp value
        e.target.value = value; // Update input with clamped value
        if (sliderElement) sliderElement.value = value;
        updateColorPreviews();
    }
    const themeSliders = [bgRedSlider, bgGreenSlider, bgBlueSlider, textRedSlider, textGreenSlider, textBlueSlider];
    const themeInputs = [bgRedInput, bgGreenInput, bgBlueInput, textRedInput, textGreenInput, textBlueInput];
    function addThemeListeners() {
        themeSliders.forEach(slider => slider?.addEventListener('input', handleThemeSliderInput));
        themeInputs.forEach(input => input?.addEventListener('input', handleThemeNumberInput));
    }
    function removeThemeListeners() {
        themeSliders.forEach(slider => slider?.removeEventListener('input', handleThemeSliderInput));
        themeInputs.forEach(input => input?.removeEventListener('input', handleThemeNumberInput));
    }
    // --- End Theme Editor Specific Logic ---

    function handleSaveEditedSettings(){
        console.log("Saving edits..."); let changesMade = false; let needsUIUpdate = false; const cLI = state.live.currentLevelIndex; let overallValid = true;
        // --- Save Blinds ---
        const newBlindLevels = []; const editBlindRows = editBlindStructureBody.querySelectorAll('tr'); if(editBlindRows.length===0){alert("Minst ett nivå kreves.");return;}
        editBlindRows.forEach((row,index)=>{ /* ... (Validation) ... */ const levelNum=index+1; let rowValid=true; const sbInput=row.querySelector('.sb-input'); const bbInput=row.querySelector('.bb-input'); const anteInput=row.querySelector('.ante-input'); const durationInput=row.querySelector('.duration-input'); const pauseInput=row.querySelector('.pause-duration-input'); const sb=parseInt(sbInput.value); const bb=parseInt(bbInput.value); const ante=parseInt(anteInput.value)||0; const duration=parseInt(durationInput.value); const pauseMinutes=parseInt(pauseInput.value)||0; const isPastLevel=levelNum<=cLI+1&&!state.live.isOnBreak; [sbInput,bbInput,anteInput,durationInput,pauseInput].forEach(el=>el.classList.remove('invalid')); if(!isPastLevel){if(isNaN(duration)||duration<=0){rowValid=false;durationInput.classList.add('invalid');} if(isNaN(sb)||sb<0){rowValid=false;sbInput.classList.add('invalid');} if(isNaN(bb)||bb<=0){rowValid=false;bbInput.classList.add('invalid');} else if(bb>0&&sb<=0){rowValid=false;sbInput.classList.add('invalid');} else if(bb>0&&sb!==roundToNearestValid(Math.floor(bb/2),100)){rowValid=false;sbInput.classList.add('invalid'); bbInput.classList.add('invalid');} if(isNaN(ante)||ante<0){rowValid=false;anteInput.classList.add('invalid');} if(isNaN(pauseMinutes)||pauseMinutes<0){rowValid=false;pauseInput.classList.add('invalid');}} if(!rowValid)overallValid=false; newBlindLevels.push({level:levelNum,sb:sb,bb:bb,ante:ante,duration:duration,pauseMinutes:pauseMinutes});});
        ...
        if(!overallValid||!prizesValid){console.warn("Edits not saved.");return;} // Stop if any part failed validation or user cancel
        ...
        if (finalBgColor !== originalThemeBg || finalTextColor !== originalThemeText) {
             ...
             logActivity(state.live.activityLog, "Fargetema endret.");
             changesMade = true; // Indicate change even if only theme changed
             applyThemeColors(finalBgColor, finalTextColor); // Ensure final theme is applied
        }

        // --- Finalize ---
        if(changesMade){if(saveTournamentState(currentTournamentId,state)){alert("Endringer lagret!"); if(needsUIUpdate)updateUI(); closeEditModal(false);}else{alert("Lagring feilet!");}}else{alert("Ingen endringer å lagre."); closeEditModal(false);}
    }
