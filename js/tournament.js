...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let state = null; // Loaded in validation section
    ...
    const playersRemainingDisplay = document.getElementById('players-remaining'); /*const playersStartedDisplay = document.getElementById('players-started');*/ // Removed this, using entries
    const totalEntriesDisplay = document.getElementById('total-entries'); const averageStackDisplay = document.getElementById('average-stack');
    const totalPotDisplay = document.getElementById('total-pot'); const lateRegStatusDisplay = document.getElementById('late-reg-status');
    const prizeDisplayLive = document.getElementById('prize-display-live'); const startPauseButton = document.getElementById('btn-start-pause');
    ...
    const addPlayerNameInput = document.getElementById('add-player-name-live'); const btnAddPlayerLive = document.getElementById('btn-add-player-live');
    ...
    // Ensure new state properties exist if loading older data
    if (state.live.isOnBreak === undefined) state.live.isOnBreak = false;
    if (state.live.timeRemainingInBreak === undefined) state.live.timeRemainingInBreak = 0;
    if (!state.live.knockoutLog) state.live.knockoutLog = []; // Ensure log exists
    ...
    function balanceTables() {
         ...
         if (balancingPerformed) { console.log("Balancing check complete."); state.live.players.sort((a, b) => a.table === b.table ? a.seat - b.seat : a.table - b.table); return true; } // Indicate change happened
    ...
    function renderPlayerList() { /* ... (Same as previous attempt) ... */
    ...
    function displayPrizes() { /* ... (same as before) ... */ if (!prizeDisplayLive) return; prizeDisplayLive.innerHTML = '<h3>Premier (Estimat)</h3>'; const prizeData = calculatePrizes(); if (prizeData.length > 0) { const ol = document.createElement('ol'); prizeData.forEach(p => { const li = document.createElement('li'); li.textContent = `${p.place}. Plass: ${p.amount.toLocaleString('nb-NO')} kr (${p.percentage}%)`; ol.appendChild(li); }); prizeDisplayLive.appendChild(ol); } else { const potForPrizes = state.live.totalPot - (state.config.type === 'knockout' ? state.live.totalEntries * (state.config.bountyAmount || 0) : 0); if (potForPrizes > 0 && (!state.config.prizeDistribution || state.config.prizeDistribution.length === 0)) { prizeDisplayLive.innerHTML += '<p>Premiefordeling ikke definert.</p>'; } else if (potForPrizes <= 0) { prizeDisplayLive.innerHTML += '<p>Ingen premiepott å fordele ennå.</p>'; } else { prizeDisplayLive.innerHTML += '<p>Beregner...</p>'; } } }
    function updateUI() {
         ...
         const upcomingPauseMinutes = currentLevel?.pauseMinutes || 0;
         const nextLevelIsBreak = upcomingPauseMinutes > 0;
         ...
         if (state.live.isOnBreak) { // Handle break display
             timerDisplay.textContent = formatTime(state.live.timeRemainingInBreak);
             blindsDisplay.innerHTML = ''; blindsDisplay.classList.add('hidden');
             breakInfo.classList.remove('hidden'); breakInfo.textContent = `PAUSE (${formatTime(state.live.timeRemainingInBreak)})`;
         } else { // Handle level display
             timerDisplay.textContent = formatTime(state.live.timeRemainingInLevel);
             blindsDisplay.classList.remove('hidden'); breakInfo.classList.add('hidden');
             blindsDisplay.innerHTML = formatBlindsHTML(currentLevel);
         }
         ...
         btnAddPlayerLive.disabled = !canAddGeneric; addPlayerNameInput.disabled = !canAddGeneric;

         startPauseButton.textContent = state.live.status === 'running' ? 'Pause Klokke' : 'Start Klokke';
         startPauseButton.disabled = state.live.status === 'finished';
    ...
    function tick() {
        ...
        if (state.live.isOnBreak) {
            ...
            breakInfo.textContent = `PAUSE (${formatTime(state.live.timeRemainingInBreak)})`; // Update break timer display
            ...
            if (state.live.timeRemainingInBreak < 0) {
                ...
                console.log(`Pause over. Nivå ${newLevel.level} startet: ${formatNextBlindsText(newLevel)}`);
                updateUI(); // Full update needed
                saveTournamentState(currentTournamentId, state);
            } else if (state.live.timeRemainingInBreak % 15 === 0) { // Save during break occasionally
                 saveTournamentState(currentTournamentId, state);
            }
            ...
            timerDisplay.textContent = formatTime(state.live.timeRemainingInLevel); // Update level timer display
            ...
            if (state.live.timeRemainingInLevel < 0) {
                ...
                if (pauseMinutes > 0) {
                    ...
                    console.log(`Nivå ${currentLevel.level} ferdig. Starter ${pauseMinutes} min pause.`);
                    updateUI(); // Full update needed
                    saveTournamentState(currentTournamentId, state);
                } else {
            ...
            } else if (state.live.timeRemainingInLevel > 0 && state.live.timeRemainingInLevel % 30 === 0) { // Save during level occasionally
                 saveTournamentState(currentTournamentId, state);
            }
        }
    }
    ...
    function handleEliminate(event) {
        ...
        if (confirm(`Eliminere ${player.name}?`)) {
            ...
            const balanced = balanceTables(); // Check balance, returns true if changes were made
            if (!balanced) { // If balance wasn't needed/done, trigger UI update and save here
                 updateUI();
                 saveTournamentState(currentTournamentId, state);
            } // If balanced, updateUI and save happens inside balanceTables
            if (state.live.players.length <= 1) { // Check finish *after* potential UI updates/saves
                 finishTournament();
            }
        }
    }
    ...
    function handleAddPlayerLive() { if (state.live.status === 'finished') { alert("Turnering er fullført."); return; } const name = addPlayerNameInput.value.trim(); if (name === "") { alert("Skriv inn navn."); return; } const cost = state.config.buyIn; if (confirm(`Legge til "${name}" (Buy-in: ${cost} kr)?`)) { const player = { id: generateUniqueId('p'), name: name, stack: state.config.startStack, table: 0, seat: 0, rebuys: 0, addon: false, eliminated: false, eliminatedBy: null, place: null, knockouts: 0 }; state.live.players.push(player); assignTableSeat(player); state.live.totalPot += cost; state.live.totalEntries++; addPlayerNameInput.value = ''; const balanced = balanceTables(); if (!balanced) { updateUI(); saveTournamentState(currentTournamentId, state); } } }
    ...
    function addEditBlindLevelRow(levelData = {}) { // Separate for modal, includes pause input
        ...
        const isPastLevel = levelData.level <= state.live.currentLevelIndex + 1 && !state.live.isOnBreak; // Lock past *levels*, allow editing pause for current level maybe? Let's lock pause too for simplicity.
    ...
    function handleSaveEditedSettings() {
         ...
         editBlindRows.forEach((row, index) => {
             ...
             const sb = parseInt(row.querySelector('.sb-input').value); const bb = parseInt(row.querySelector('.bb-input').value); const ante = parseInt(row.querySelector('.ante-input').value) || 0; const duration = parseInt(row.querySelector('.duration-input').value); const pauseMinutes = parseInt(row.querySelector('.pause-duration-input').value) || 0;
             // Validate only editable rows (future levels)
             const isPastLevel = levelNum <= currentLevelIndex + 1; // Simple check if level number is past/current
             if (!isPastLevel) { // Only validate future levels
                 if (isNaN(duration) || duration <= 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig varighet.`); }
                 if (isNaN(sb) || sb < 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig SB.`); }
                 if (isNaN(bb) || bb <= 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig BB.`); }
                 else if (bb > 0 && sb <= 0) { rowValid = false; alert(`Nivå ${levelNum}: SB må være > 0 hvis BB > 0.`); } // Enforce SB > 0 if BB > 0
                 else if (bb > 0 && sb !== roundToNearestValid(Math.floor(bb/2), 100)) { rowValid = false; alert(`Nivå ${levelNum}: SB (${sb}) er ikke ~50% av BB (${bb}) og rundet.`);}
                 if (isNaN(ante) || ante < 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig Ante.`); }
                 if (isNaN(pauseMinutes) || pauseMinutes < 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig pause.`); }
             }
             if (!rowValid) overallValid = false;
             newBlindLevels.push({ level: levelNum, sb: sb, bb: bb, ante: ante, duration: duration, pauseMinutes: pauseMinutes }); // Store original level number
         });
         if (!overallValid) { console.warn("Blinds edits not saved due to validation errors."); return; } // Stop if validation failed
    ...
    function finishTournament() {
        ...
        clearInterval(timerInterval); timerInterval = null; clearInterval(realTimeInterval); realTimeInterval = null; // Stop both timers
        state.live.status = 'finished'; state.live.isOnBreak = false; // Ensure not stuck on break
        ...
        else if (state.live.players.length === 0) { console.log("Finished with no active players."); }
        else { console.log(`Finished with ${state.live.players.length} players remaining (Chop?).`); }
        state.live.eliminatedPlayers.sort((a, b) => (a.place ?? Infinity) - (b.place ?? Infinity));
        console.log("Tournament officially finished!");
        updateUI(); // Update UI to final state (includes disabling buttons)
    ...
    lateRegButton.addEventListener('click', handleLateRegClick); btnAddPlayerLive.addEventListener('click', handleAddPlayerLive); endTournamentButton.addEventListener('click', handleEndTournament); btnForceSave.addEventListener('click', handleForceSave); btnBackToMainLive.addEventListener('click', handleBackToMain);
    ...
    else if (state.live.status === 'finished') { finishTournament(); } // Ensure UI is fully updated and timers stopped if loaded finished
    // === 15: INITIAL UI RENDER & TIMER START ===
