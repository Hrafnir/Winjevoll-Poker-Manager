...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const prizeDisplayLive = document.getElementById('prize-display-live'); const prizeSummarySpan = document.getElementById('prize-summary'); // Ref for new summary span
    ...
    const adjustTimeMinusButton = document.getElementById('btn-adjust-time-minus'); const adjustTimePlusButton = document.getElementById('btn-adjust-time-plus');
    ...
    state.live = state.live || {}; // Ensure live object exists
    ...
    state.live.eliminatedPlayers = state.live.eliminatedPlayers || []; // Ensure eliminated list exists
    state.live.players = state.live.players || []; // Ensure player list exists
    ...
    function formatBlindsHTML(level) { if (!level) return `<span class="value">--</span> / <span class="value">--</span> <span class="label">Ante:</span> <span class="value">--</span>`; let anteHtml = ''; if (level.ante > 0) anteHtml = `<span class="label">Ante:</span> <span class="value">${level.ante}</span>`; return `<span class="value">${level.sb}</span> / <span class="value">${level.bb}</span> ${anteHtml}`; }
    function formatNextBlindsText(level, nextIsBreak = false, breakDuration = 0) { if (!level && !nextIsBreak) return "N/A"; if (nextIsBreak) return `Pause (${breakDuration} min)`; const anteText = level.ante > 0 ? ` (${level.ante} Ante)` : ''; return `${level.sb} / ${level.bb}${anteText}`; }
    ...
    function calculatePrizes() { /* ... (Same as before) ... */ const prizes = []; const potForPrizes = state.live.totalPot - (state.config.type === 'knockout' ? state.live.totalEntries * (state.config.bountyAmount || 0) : 0); if (potForPrizes <= 0 || !state.config.prizeDistribution || state.config.prizeDistribution.length === 0 || state.config.prizeDistribution.length !== state.config.paidPlaces) { return prizes; } let sumOfPrizesCalculated = 0; for (let i = 0; i < state.config.paidPlaces; i++) { const percentage = state.config.prizeDistribution[i] || 0; let amount; if (i === state.config.paidPlaces - 1) { amount = Math.max(0, potForPrizes - sumOfPrizesCalculated); } else { amount = Math.floor((potForPrizes * percentage) / 100); } if (amount > 0 || (prizes.length === 0 && state.config.paidPlaces === 1)) { prizes.push({ place: i + 1, amount: amount, percentage: percentage }); sumOfPrizesCalculated += amount; } else if (prizes.length > 0) { prizes.push({ place: i + 1, amount: 0, percentage: percentage }); } else { break; } if(prizes.length >= state.config.paidPlaces) break; } return prizes; }
    // === 06: HELPER FUNCTIONS - CALCULATIONS END ===


    // === 07: HELPER FUNCTIONS - TABLE MANAGEMENT START ===
    function assignTableSeat(player) { /* ... (Same as before) ... */ const tables = {}; let minTableNum = -1; state.live.players.forEach(p => { if (p.id !== player.id) { tables[p.table] = (tables[p.table] || 0) + 1; } }); const sortedTables = Object.entries(tables).map(([num, count]) => ({ tableNum: parseInt(num), count: count })).sort((a, b) => a.count - b.count); for (const tableInfo of sortedTables) { if (tableInfo.count < state.config.playersPerTable) { minTableNum = tableInfo.tableNum; break; } } if (minTableNum === -1) { const existingTableNumbers = Object.keys(tables).map(Number); minTableNum = existingTableNumbers.length > 0 ? Math.max(...existingTableNumbers) + 1 : 1; player.table = minTableNum; player.seat = 1; } else { const occupiedSeats = state.live.players.filter(p => p.table === minTableNum).map(p => p.seat); let seatNum = 1; while (occupiedSeats.includes(seatNum)) { seatNum++; } if (seatNum > state.config.playersPerTable) { console.error(`ERROR: No seat on table ${minTableNum}`); seatNum = occupiedSeats.length + 1; } player.table = minTableNum; player.seat = seatNum; } console.log(`Assigned ${player.name} to T${player.table} S${player.seat}`); }
    function balanceTables() { // Returns true if balancing was done
         ...
         if (balancingPerformed) { console.log("Balancing changes applied."); state.live.players.sort((a, b) => a.table === b.table ? a.seat - b.seat : a.table - b.table); return true; } // Indicate change
    ...
    function displayPrizes() { /* ... (Update to include summary span) ... */
        ...
        if(prizeSummarySpan) prizeSummarySpan.textContent = summaryText; // Update summary span
    }
    function updateUI() {
         ...
         lateRegButton.disabled = !lateRegOpen || state.live.status !== 'running'; // Disable if paused or finished or not late reg
         /* Removed generic add button disabling */
    ...
    function handleStartPause() { if (state.live.status === 'finished') return; if (state.live.status === 'paused') { state.live.status = 'running'; if (!timerInterval) timerInterval = setInterval(tick, 1000); } else { state.live.status = 'paused'; saveTournamentState(currentTournamentId, state); } updateUI(); }
    function handleAdjustTime(deltaSeconds) { if (state.live.status === 'finished') return; if (state.live.isOnBreak) { state.live.timeRemainingInBreak += deltaSeconds; state.live.timeRemainingInBreak = Math.max(0, state.live.timeRemainingInBreak); } else { state.live.timeRemainingInLevel += deltaSeconds; state.live.timeRemainingInLevel = Math.max(0, state.live.timeRemainingInLevel); const currentLevelDuration = state.config.blindLevels[state.live.currentLevelIndex]?.duration * 60; if (currentLevelDuration) state.live.timeRemainingInLevel = Math.min(currentLevelDuration, state.live.timeRemainingInLevel); } updateUI(); saveTournamentState(currentTournamentId, state); }
    function handleAdjustLevel(deltaIndex) { if (state.live.status === 'finished') return; const newIndex = state.live.currentLevelIndex + deltaIndex; if (newIndex >= 0 && newIndex < state.config.blindLevels.length) { state.live.currentLevelIndex = newIndex; const newLevel = state.config.blindLevels[newIndex]; state.live.timeRemainingInLevel = newLevel.duration * 60; state.live.isOnBreak = false; state.live.timeRemainingInBreak = 0; updateUI(); saveTournamentState(currentTournamentId, state); } else { console.warn("Cannot adjust level."); } }
    ...
    function handleRebuy(event) { /* ... (No changes needed) ... */ const playerId = parseInt(event.target.dataset.playerId); const player = state.live.players.find(p => p.id === playerId); const currentLevelNum = state.live.currentLevelIndex + 1; if (!player || state.config.type !== 'rebuy' || !(currentLevelNum <= state.config.rebuyLevels)) { alert("Re-buy ikke tilgjengelig."); return; } if (confirm(`Re-buy (${state.config.rebuyCost} kr) for ${player.name}?`)) { player.rebuys = (player.rebuys || 0) + 1; state.live.totalPot += state.config.rebuyCost; state.live.totalEntries++; state.live.totalRebuys++; updateUI(); saveTournamentState(currentTournamentId, state); } }
    function handleAddon(event) { /* ... (No changes needed) ... */ const playerId = parseInt(event.target.dataset.playerId); const player = state.live.players.find(p => p.id === playerId); const currentLevelNum = state.live.currentLevelIndex + 1; const isAddonPeriod = currentLevelNum > state.config.rebuyLevels; if (!player || state.config.type !== 'rebuy' || !isAddonPeriod || player.addon) { alert("Add-on ikke tilgjengelig."); return; } if (confirm(`Add-on (${state.config.addonCost} kr) for ${player.name}?`)) { player.addon = true; state.live.totalPot += state.config.addonCost; state.live.totalAddons++; updateUI(); saveTournamentState(currentTournamentId, state); } }
    function handleEliminate(event) { // Handles save/update internally if balanceTables doesn't
        if (state.live.status === 'finished') return; const playerId = parseInt(event.target.dataset.playerId); const playerIndex = state.live.players.findIndex(p => p.id === playerId); if (playerIndex === -1) return; const player = state.live.players[playerIndex]; let eliminatedByPlayerId = null;
        if (state.config.type === 'knockout' && (state.config.bountyAmount || 0) > 0 && state.live.players.length > 1) { const activePlayerNames = state.live.players.filter(p => p.id !== playerId).map(p => `${p.name} (B${p.table}S${p.seat})`); const promptMessage = `Hvem slo ut ${player.name}?\n\nAktive:\n - ${activePlayerNames.join("\n - ")}`; const eliminatorInput = prompt(promptMessage); if (eliminatorInput?.trim()) { const eliminatorName = eliminatorInput.trim().toLowerCase(); const eliminator = state.live.players.find(p => p.id !== playerId && p.name.toLowerCase() === eliminatorName); if (eliminator) { eliminatedByPlayerId = eliminator.id; eliminator.knockouts = (eliminator.knockouts || 0) + 1; state.live.knockoutLog.push({ eliminatedPlayerId: player.id, eliminatedByPlayerId: eliminator.id }); } else { alert(`Fant ikke "${eliminatorInput}".`); } } }
        if (confirm(`Eliminere ${player.name}?`)) { player.eliminated = true; player.eliminatedBy = eliminatedByPlayerId; player.place = state.live.players.length; state.live.eliminatedPlayers.push(player); state.live.players.splice(playerIndex, 1);
            const balanced = balanceTables(); // balanceTables calls save/update if it does anything
            if (!balanced) { updateUI(); saveTournamentState(currentTournamentId, state); } // Manual save/update if no balancing occurred
            if (state.live.players.length <= 1) { finishTournament(); } // Check finish AFTER potential updates
        }
    }
    function handleRestore(event) { // Handles save/update internally if balanceTables doesn't
    ...
    function handleEditPlayer(event) { /* ... (No changes needed) ... */ if (state.live.status === 'finished') return; const playerId = parseInt(event.target.dataset.playerId); const player = state.live.players.find(p => p.id === playerId); if (!player) return; const newName = prompt(`Endre navn for ${player.name}:`, player.name); if (newName?.trim()) { player.name = newName.trim(); renderPlayerList(); saveTournamentState(currentTournamentId, state); } else if (newName === "") alert("Navn kan ikke være tomt."); }
    /* Removed handleAddPlayerLive - now combined with handleLateRegClick conceptually */
    function handleLateRegClick() { // Handles Late Reg button press
        ...
        if (state.live.status !== 'running') { alert("Start klokken for sen registrering."); return; }
        ...
        if (name?.trim()) {
            ...
            state.live.players.push(player); assignTableSeat(player); state.live.totalPot += cost; state.live.totalEntries++;
            const balanced = balanceTables(); if (!balanced) { updateUI(); saveTournamentState(currentTournamentId, state); }
        }
    }
    ...
    function openEditModal() { /* ... (No changes needed) ... */ if (state.live.status === 'finished' || isModalOpen) return; console.log("Opening edit modal"); editBlindStructureBody.innerHTML = ''; editBlindLevelCounter = 0; state.config.blindLevels.forEach(level => addEditBlindLevelRow(level)); updateEditLevelNumbers(); editPaidPlacesInput.value = state.config.paidPlaces; editPrizeDistTextarea.value = state.config.prizeDistribution.join(', '); modal.classList.remove('hidden'); isModalOpen = true; }
    function closeEditModal() { /* ... (No changes needed) ... */ modal.classList.add('hidden'); isModalOpen = false; }
    function addEditBlindLevelRow(levelData = {}) { // Updated for pauseMinutes input
        ...
        const isPastLevel = levelData.level <= state.live.currentLevelIndex + 1 && !state.live.isOnBreak;
        row.innerHTML = `<td><span class="level-number">${editBlindLevelCounter}</span> ${isPastLevel ? '<small>(Låst)</small>':''}</td><td><input type="number" class="sb-input" value="${sb}" min="0" step="100" ${isPastLevel ? 'disabled' : ''}></td><td><input type="number" class="bb-input" value="${bb}" min="0" step="100" ${isPastLevel ? 'disabled' : ''}></td><td><input type="number" class="ante-input" value="${ante}" min="0" step="25" ${isPastLevel ? 'disabled' : ''}></td><td><input type="number" class="duration-input" value="${duration}" min="1" ${isPastLevel ? 'disabled' : ''}></td><td><input type="number" class="pause-duration-input" value="${pauseMinutes}" min="0" ${isPastLevel ? 'disabled' : ''}></td><td><button type="button" class="btn-remove-level" title="Fjern nivå ${editBlindLevelCounter}" ${isPastLevel ? 'disabled' : ''}>X</button></td>`;
        const removeBtn = row.querySelector('.btn-remove-level'); const bbInput = row.querySelector('.bb-input'); const sbInput = row.querySelector('.sb-input');
        if (!isPastLevel) { removeBtn.onclick = () => { row.remove(); updateEditLevelNumbers(); }; bbInput.onchange = () => { const bbVal = parseInt(bbInput.value); if (!isNaN(bbVal) && bbVal > 0) sbInput.value = roundToNearestValid(Math.floor(bbVal / 2), 100); }; }
        else { row.querySelectorAll('input').forEach(inp => inp.disabled = true); removeBtn.disabled = true; }
    }
    function updateEditLevelNumbers() { /* ... (No changes needed) ... */ const rows = editBlindStructureBody.querySelectorAll('tr'); rows.forEach((row, index) => { const levelNum = index + 1; row.dataset.levelNumber = levelNum; row.querySelector('.level-number').textContent = levelNum; row.querySelector('.btn-remove-level').title = `Fjern nivå ${levelNum}`; }); editBlindLevelCounter = rows.length; }
    function generateEditPayout() { /* ... (No changes needed) ... */ const places = parseInt(editPaidPlacesInput.value) || 0; if (places > 0 && standardPayouts[places]) { editPrizeDistTextarea.value = standardPayouts[places].join(', '); } else { editPrizeDistTextarea.value = ''; } }
    function handleSaveEditedSettings() { // Updated validation for SB/BB 50% rule
         ...
         editBlindRows.forEach((row, index) => {
             const levelNum = index + 1; let rowValid = true;
             ...
             const isPastLevel = levelNum <= currentLevelIndex + 1 && !state.live.isOnBreak;
             [sbInput, bbInput, anteInput, durationInput, pauseInput].forEach(el => el.classList.remove('invalid')); // Reset styles

             if (!isPastLevel) { // Only validate future levels
                 if (isNaN(duration) || duration <= 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig varighet.`); durationInput.classList.add('invalid'); }
                 if (isNaN(sb) || sb < 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig SB.`); sbInput.classList.add('invalid');}
                 if (isNaN(bb) || bb <= 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig BB (>0).`); bbInput.classList.add('invalid');}
                 else if (bb > 0 && sb <= 0) { rowValid = false; alert(`Nivå ${levelNum}: SB må være > 0 hvis BB > 0.`); sbInput.classList.add('invalid');}
                 else if (bb > 0 && sb !== roundToNearestValid(Math.floor(bb/2), 100)) { rowValid = false; alert(`Nivå ${levelNum}: SB (${sb}) er ikke ~50% av BB (${bb}) og rundet.`); sbInput.classList.add('invalid'); bbInput.classList.add('invalid'); }
                 if (isNaN(ante) || ante < 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig Ante.`); anteInput.classList.add('invalid');}
                 if (isNaN(pauseMinutes) || pauseMinutes < 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig pause.`); pauseInput.classList.add('invalid');}
             }
         ...
         if (!overallValid) { console.warn("Blinds edits not saved due to validation errors."); return; }
         if (JSON.stringify(state.config.blindLevels) !== JSON.stringify(newBlindLevels)) { state.config.blindLevels = newBlindLevels; changesMade = true; needsUIUpdate = true; console.log("Blind structure updated."); }
    ...
    function finishTournament() {
        if (state.live.status === 'finished') return; console.log("Finishing tournament...");
        clearInterval(timerInterval); timerInterval = null; clearInterval(realTimeInterval); realTimeInterval = null;
        state.live.status = 'finished'; state.live.isOnBreak = false;
        if (state.live.players.length === 1) { const winner = state.live.players[0]; winner.place = 1; state.live.eliminatedPlayers.push(winner); state.live.players.splice(0, 1); }
        else if (state.live.players.length === 0 && state.live.eliminatedPlayers.length > 0) { console.log("Finished with no active players left."); } // Last player eliminated case
        else { console.log(`Finished with ${state.live.players.length} players remaining (Chop?).`); } // Chop / Error case
        state.live.eliminatedPlayers.sort((a, b) => (a.place ?? Infinity) - (b.place ?? Infinity));
        console.log("Tournament officially finished!");
        updateUI(); // Update UI to final state
        saveTournamentState(currentTournamentId, state); // Save final state
    ...
    startPauseButton.addEventListener('click', handleStartPause); prevLevelButton.addEventListener('click', () => handleAdjustLevel(-1)); nextLevelButton.addEventListener('click', () => handleAdjustLevel(1)); adjustTimeMinusButton.addEventListener('click', () => handleAdjustTime(-60)); adjustTimePlusButton.addEventListener('click', () => handleAdjustTime(60));
    lateRegButton.addEventListener('click', handleLateRegClick); /* Removed listener for generic add button */ endTournamentButton.addEventListener('click', handleEndTournament); btnForceSave.addEventListener('click', handleForceSave); btnBackToMainLive.addEventListener('click', handleBackToMain);
    ...
    // Player listeners added in renderPlayerList
