...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnCancelEditSettings = document.getElementById('btn-cancel-edit-settings'); const activityLogUl = document.getElementById('activity-log-list');
    ...
    function formatNextBlindsText(level, nextIsBreak = false, breakDuration = 0) { if (!level && !nextIsBreak) return "Slutt"; if (nextIsBreak) return `Pause (${breakDuration} min)`; const a = level.ante > 0 ? `(${level.ante})` : ''; return `${level.sb}/${level.bb}${a}`; }
...
// (updateUI, renderPlayerList, displayPrizes functions follow)
    ...
    function handleSaveEditedSettings(){console.log("Saving edits..."); let changesMade=false; let needsUIUpdate=false; const currentLevelIndex=state.live.currentLevelIndex; let overallValid=true; const newBlindLevels=[]; const editBlindRows=editBlindStructureBody.querySelectorAll('tr'); if(editBlindRows.length===0){alert("Minst ett nivå kreves.");return;}
    ...
    function finishTournament() { if(state.live.status==='finished')return; console.log("Finishing tournament..."); clearInterval(timerInterval); timerInterval=null; clearInterval(realTimeInterval); realTimeInterval=null; state.live.status='finished'; state.live.isOnBreak=false; if(state.live.players.length===1){const winner=state.live.players[0]; winner.place=1; state.live.eliminatedPlayers.push(winner); state.live.players.splice(0,1); logActivity(state.live.activityLog,`Vinner: ${winner.name}!`);} else if(state.live.players.length===0){logActivity(state.live.activityLog,`Turnering fullført (ingen aktive).`);} else{logActivity(state.live.activityLog,`Turnering fullført (${state.live.players.length} igjen - Chop?).`);} state.live.eliminatedPlayers.sort((a,b)=>(a.place??Infinity)-(b.place??Infinity)); console.log("Tournament officially finished!"); updateUI(); saveTournamentState(currentTournamentId,state); alert("Turneringen er fullført!"); }
