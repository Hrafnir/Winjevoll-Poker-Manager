...
document.addEventListener('DOMContentLoaded', () => {
    ...
    state.live = state.live || {}; state.live.isOnBreak = state.live.isOnBreak ?? false; state.live.timeRemainingInBreak = state.live.timeRemainingInBreak ?? 0; state.live.knockoutLog = state.live.knockoutLog || []; state.live.eliminatedPlayers = state.live.eliminatedPlayers || []; state.live.players = state.live.players || [];
    ...
    function tick() { /* ... (No logical changes needed here) ... */ if (state.live.status !== 'running') return; if (state.live.isOnBreak) { state.live.timeRemainingInBreak--; breakInfo.textContent = `PAUSE (${formatTime(state.live.timeRemainingInBreak)})`; if (state.live.timeRemainingInBreak < 0) { state.live.isOnBreak = false; state.live.currentLevelIndex++; if (state.live.currentLevelIndex >= state.config.blindLevels.length) { finishTournament(); return; } const newLevel = state.config.blindLevels[state.live.currentLevelIndex]; state.live.timeRemainingInLevel = newLevel.duration * 60; console.log(`Pause over. Nivå ${newLevel.level} startet.`); updateUI(); saveTournamentState(currentTournamentId, state); } else if (state.live.timeRemainingInBreak % 15 === 0) { saveTournamentState(currentTournamentId, state); } } else { state.live.timeRemainingInLevel--; timerDisplay.textContent = formatTime(state.live.timeRemainingInLevel); if (state.live.timeRemainingInLevel < 0) { const currentLevel = state.config.blindLevels[state.live.currentLevelIndex]; const pauseMinutes = currentLevel?.pauseMinutes || 0; if (pauseMinutes > 0) { state.live.isOnBreak = true; state.live.timeRemainingInBreak = pauseMinutes * 60; console.log(`Nivå ${currentLevel.level} ferdig. Starter ${pauseMinutes} min pause.`); updateUI(); saveTournamentState(currentTournamentId, state); } else { state.live.currentLevelIndex++; if (state.live.currentLevelIndex >= state.config.blindLevels.length) { finishTournament(); return; } const newLevel = state.config.blindLevels[state.live.currentLevelIndex]; state.live.timeRemainingInLevel = newLevel.duration * 60; console.log(`Nivå ${newLevel.level} startet.`); updateUI(); saveTournamentState(currentTournamentId, state); } } else if (state.live.timeRemainingInLevel > 0 && state.live.timeRemainingInLevel % 30 === 0) { saveTournamentState(currentTournamentId, state); } } }
    function startRealTimeClock() { /* ... (No logical changes needed here) ... */ if (realTimeInterval) clearInterval(realTimeInterval); realTimeInterval = setInterval(() => { if(currentTimeDisplay) currentTimeDisplay.textContent = new Date().toLocaleTimeString('nb-NO', { hour: '2-digit', minute: '2-digit', second: '2-digit' }); }, 1000); }
    // === 09: TIMER LOGIC END ===


    // === 10: EVENT HANDLERS - CONTROLS START ===
    function handleStartPause() { /* ... (No logical changes needed here) ... */ if (state.live.status === 'finished') return; if (state.live.status === 'paused') { state.live.status = 'running'; if (!timerInterval) timerInterval = setInterval(tick, 1000); } else { state.live.status = 'paused'; saveTournamentState(currentTournamentId, state); } updateUI(); }
    function handleAdjustTime(deltaSeconds) { /* ... (No logical changes needed here) ... */ if (state.live.status === 'finished') return; if (state.live.isOnBreak) { state.live.timeRemainingInBreak += deltaSeconds; state.live.timeRemainingInBreak = Math.max(0, state.live.timeRemainingInBreak); } else { state.live.timeRemainingInLevel += deltaSeconds; state.live.timeRemainingInLevel = Math.max(0, state.live.timeRemainingInLevel); const currentLevelDuration = state.config.blindLevels[state.live.currentLevelIndex]?.duration * 60; if (currentLevelDuration) state.live.timeRemainingInLevel = Math.min(currentLevelDuration, state.live.timeRemainingInLevel); } updateUI(); saveTournamentState(currentTournamentId, state); }
    function handleAdjustLevel(deltaIndex) { /* ... (No logical changes needed here) ... */ if (state.live.status === 'finished') return; const newIndex = state.live.currentLevelIndex + deltaIndex; if (newIndex >= 0 && newIndex < state.config.blindLevels.length) { state.live.currentLevelIndex = newIndex; const newLevel = state.config.blindLevels[newIndex]; state.live.timeRemainingInLevel = newLevel.duration * 60; state.live.isOnBreak = false; state.live.timeRemainingInBreak = 0; updateUI(); saveTournamentState(currentTournamentId, state); } else { console.warn("Cannot adjust level."); } }
    function handleEndTournament() { /* ... (No logical changes needed here) ... */ if (state.live.status === 'finished') { alert("Turneringen er allerede fullført."); return; } if (confirm("Markere turneringen som fullført?")) finishTournament(); }
    function handleForceSave() { /* ... (No logical changes needed here) ... */ if(state) { if(saveTournamentState(currentTournamentId, state)) { btnForceSave.textContent = "Lagret!"; setTimeout(() => { btnForceSave.textContent = "Lagre Nå"; }, 1500); } else { alert("Lagring feilet!");} } }
    function handleBackToMain() { /* ... (No logical changes needed here) ... */ if (state && state.live.status !== 'finished') saveTournamentState(currentTournamentId, state); window.location.href = 'index.html'; }
    ...
    function openEditModal() { /* ... (No logical changes needed here) ... */ if (state.live.status === 'finished' || isModalOpen) return; console.log("Opening edit modal"); editBlindStructureBody.innerHTML = ''; editBlindLevelCounter = 0; state.config.blindLevels.forEach(level => addEditBlindLevelRow(level)); updateEditLevelNumbers(); editPaidPlacesInput.value = state.config.paidPlaces; editPrizeDistTextarea.value = state.config.prizeDistribution.join(', '); modal.classList.remove('hidden'); isModalOpen = true; }
    function closeEditModal() { /* ... (No logical changes needed here) ... */ modal.classList.add('hidden'); isModalOpen = false; }
    function addEditBlindLevelRow(levelData = {}) { /* ... (No logical changes needed here, uses pauseMinutes) ... */ editBlindLevelCounter++; const row = editBlindStructureBody.insertRow(); row.dataset.levelNumber = editBlindLevelCounter; const sb = levelData.sb ?? ''; const bb = levelData.bb ?? ''; const ante = levelData.ante ?? 0; const duration = levelData.duration ?? (state.config.blindLevels?.[0]?.duration || 20); const pauseMinutes = levelData.pauseMinutes ?? 0; const isPastLevel = levelData.level <= state.live.currentLevelIndex + 1 && !state.live.isOnBreak; row.innerHTML = `<td><span class="level-number">${editBlindLevelCounter}</span> ${isPastLevel ? '<small>(Låst)</small>':''}</td><td><input type="number" class="sb-input" value="${sb}" min="0" step="100" ${isPastLevel ? 'disabled' : ''}></td><td><input type="number" class="bb-input" value="${bb}" min="0" step="100" ${isPastLevel ? 'disabled' : ''}></td><td><input type="number" class="ante-input" value="${ante}" min="0" step="25" ${isPastLevel ? 'disabled' : ''}></td><td><input type="number" class="duration-input" value="${duration}" min="1" ${isPastLevel ? 'disabled' : ''}></td><td><input type="number" class="pause-duration-input" value="${pauseMinutes}" min="0" ${isPastLevel ? 'disabled' : ''}></td><td><button type="button" class="btn-remove-level" title="Fjern nivå ${editBlindLevelCounter}" ${isPastLevel ? 'disabled' : ''}>X</button></td>`; const removeBtn = row.querySelector('.btn-remove-level'); const bbInput = row.querySelector('.bb-input'); const sbInput = row.querySelector('.sb-input'); if (!isPastLevel) { removeBtn.onclick = () => { row.remove(); updateEditLevelNumbers(); }; bbInput.onchange = () => { const bbVal = parseInt(bbInput.value); if (!isNaN(bbVal) && bbVal > 0) sbInput.value = roundToNearestValid(Math.floor(bbVal / 2), 100); }; } else { row.querySelectorAll('input').forEach(inp => inp.disabled = true); removeBtn.disabled = true; } }
    function updateEditLevelNumbers() { /* ... (No logical changes needed here) ... */ const rows = editBlindStructureBody.querySelectorAll('tr'); rows.forEach((row, index) => { const levelNum = index + 1; row.dataset.levelNumber = levelNum; row.querySelector('.level-number').textContent = levelNum; row.querySelector('.btn-remove-level').title = `Fjern nivå ${levelNum}`; }); editBlindLevelCounter = rows.length; }
    function generateEditPayout() { /* ... (No logical changes needed here) ... */ const places = parseInt(editPaidPlacesInput.value) || 0; if (places > 0 && standardPayouts[places]) { editPrizeDistTextarea.value = standardPayouts[places].join(', '); } else { editPrizeDistTextarea.value = ''; } }
    function handleSaveEditedSettings() { /* ... (No logical changes needed here, validates pauseMinutes) ... */ console.log("Saving edited settings..."); let changesMade = false; let needsUIUpdate = false; const currentLevelIndex = state.live.currentLevelIndex; let overallValid = true; const newBlindLevels = []; const editBlindRows = editBlindStructureBody.querySelectorAll('tr'); if (editBlindRows.length === 0) { alert("Minst ett blindnivå kreves."); return; } editBlindRows.forEach((row, index) => { const levelNum = index + 1; let rowValid = true; const sbInput=row.querySelector('.sb-input'); const bbInput=row.querySelector('.bb-input'); const anteInput=row.querySelector('.ante-input'); const durationInput=row.querySelector('.duration-input'); const pauseInput=row.querySelector('.pause-duration-input'); const sb = parseInt(sbInput.value); const bb = parseInt(bbInput.value); const ante = parseInt(anteInput.value) || 0; const duration = parseInt(durationInput.value); const pauseMinutes = parseInt(pauseInput.value) || 0; const isPastLevel = levelNum <= currentLevelIndex + 1 && !state.live.isOnBreak; [sbInput, bbInput, anteInput, durationInput, pauseInput].forEach(el => el.classList.remove('invalid')); if (!isPastLevel) { if (isNaN(duration) || duration <= 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig varighet.`); durationInput.classList.add('invalid'); } if (isNaN(sb) || sb < 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig SB.`); sbInput.classList.add('invalid');} if (isNaN(bb) || bb <= 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig BB (>0).`); bbInput.classList.add('invalid');} else if (bb > 0 && sb <= 0) { rowValid = false; alert(`Nivå ${levelNum}: SB må være > 0 hvis BB > 0.`); sbInput.classList.add('invalid');} else if (bb > 0 && sb !== roundToNearestValid(Math.floor(bb/2), 100)) { rowValid = false; alert(`Nivå ${levelNum}: SB (${sb}) er ikke ~50% av BB (${bb}) og rundet.`); sbInput.classList.add('invalid'); bbInput.classList.add('invalid'); } if (isNaN(ante) || ante < 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig Ante.`); anteInput.classList.add('invalid');} if (isNaN(pauseMinutes) || pauseMinutes < 0) { rowValid = false; alert(`Nivå ${levelNum}: Ugyldig pause.`); pauseInput.classList.add('invalid');} } if (!rowValid) overallValid = false; newBlindLevels.push({ level: levelNum, sb: sb, bb: bb, ante: ante, duration: duration, pauseMinutes: pauseMinutes }); }); if (!overallValid) { console.warn("Blinds edits not saved."); return; } if (JSON.stringify(state.config.blindLevels) !== JSON.stringify(newBlindLevels)) { state.config.blindLevels = newBlindLevels; changesMade = true; needsUIUpdate = true; console.log("Blind structure updated."); } const newPaidPlaces = parseInt(editPaidPlacesInput.value); const newPrizeDist = editPrizeDistTextarea.value.split(',').map(p => parseFloat(p.trim())).filter(p => !isNaN(p) && p >= 0); let prizesValid = true; if (isNaN(newPaidPlaces) || newPaidPlaces <= 0) { prizesValid = false; alert("Ugyldig antall betalte."); } else if (newPrizeDist.length !== newPaidPlaces) { prizesValid = false; alert("Antall % != Antall betalte."); } else { const sum = newPrizeDist.reduce((a, b) => a + b, 0); if (Math.abs(sum - 100) > 0.1) { prizesValid = false; alert(`% sum (${sum.toFixed(1)}) != 100.`); } } if (prizesValid && (state.config.paidPlaces !== newPaidPlaces || JSON.stringify(state.config.prizeDistribution) !== JSON.stringify(newPrizeDist))) { const playersInMoney = state.live.eliminatedPlayers.filter(p => p.place <= state.config.paidPlaces).length; if (playersInMoney === 0 || confirm(`Advarsel: ${playersInMoney} spiller(e) i pengene. Sikker på å endre premier?`)) { state.config.paidPlaces = newPaidPlaces; state.config.prizeDistribution = newPrizeDist; changesMade = true; needsUIUpdate = true; console.log("Prize structure updated."); } else { console.log("Prize change cancelled."); prizesValid = false;} } if (!overallValid || !prizesValid) { console.warn("Edits not saved."); return; } if (changesMade) { if(saveTournamentState(currentTournamentId, state)) { alert("Endringer lagret!"); if (needsUIUpdate) updateUI(); closeEditModal(); } else { alert("Lagring av endringer feilet!"); } } else { alert("Ingen endringer å lagre."); closeEditModal(); } }
    // === 12: EVENT HANDLERS - MODAL & EDIT SETTINGS END ===


    // === 13: TOURNAMENT FINISH LOGIC START ===
    function finishTournament() { /* ... (No logical changes needed here) ... */ if (state.live.status === 'finished') return; console.log("Finishing tournament..."); clearInterval(timerInterval); timerInterval = null; clearInterval(realTimeInterval); realTimeInterval = null; state.live.status = 'finished'; state.live.isOnBreak = false; if (state.live.players.length === 1) { const winner = state.live.players[0]; winner.place = 1; state.live.eliminatedPlayers.push(winner); state.live.players.splice(0, 1); } else if (state.live.players.length === 0) { console.log("Finished with no active players."); } else { console.log(`Finished with ${state.live.players.length} players remaining (Chop?).`); } state.live.eliminatedPlayers.sort((a, b) => (a.place ?? Infinity) - (b.place ?? Infinity)); console.log("Tournament officially finished!"); updateUI(); saveTournamentState(currentTournamentId, state); alert("Turneringen er fullført!"); }
