...
// -----------------------------------------------------------------------
// All kode kjøres etter at HTML er fullstendig lastet og parset.
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // -----------------------------------------------------------------------
    // Globale variabler for denne sidens tilstand
    const currentTournamentId = getActiveTournamentId();
    let state = null; // Hovedobjektet for turneringen (config + live)
    let soundsEnabled = loadSoundPreference();
    let currentLogoBlob = null; // Hovedsidens logo blob
    let isModalOpen = false; // Holder styr på om *en* modal er åpen
    let currentOpenModal = null; // Referanse til den aktive modalen
    ...
    // UI Settings Modal spesifikk state
    ...
    let originalSettings = {}; // For 'Avbryt' i UI Settings
    let livePreviewEnabled = true;
    let isUpdatingColorControls = false; // For fargekontroll-synkronisering
    let logoClearedInModal = false; // For sletting av logo

    // Tournament Settings Modal spesifikk state
    let modalBlindLevelCounter = 0; // Egen teller for blinds i modal

    // Konstanter brukt i flere modaler/funksjoner
    ...
    // -----------------------------------------------------------------------
    // Referanser til HTML-elementer hentes én gang her for ytelse.
    // --- Generelle Elementer ---
    ...
    const draggableElements = [titleElement, timerElement, blindsElement, logoElement, infoElement].filter(Boolean); // Filtrer ut null hvis element mangler
    ...
    // -----------------------------------------------------------------------
    // Laster state, validerer data, setter defaults.
    ...
    // Grundig validering/default-setting
    ...
    // -----------------------------------------------------------------------
    // Setter opp utseende basert på lagrede innstillinger.
    ...
    // -----------------------------------------------------------------------
    // Funksjoner for å styre selve turneringen (start/pause, nivå, tid, fullføring).
    ...
    // -----------------------------------------------------------------------
    // Funksjoner for å håndtere flytting av UI-elementer.
    ...
    // -----------------------------------------------------------------------
    // Definerer et objekt med funksjoner som sendes til andre moduler.
     const mainCallbacks = {
        updateUI: () => { if (state) updateUI(state); },
        saveState: () => { if (state) saveTournamentState(currentTournamentId, state); },
        playSound: null, // TODO: Implementer playSound
    ...
    // -----------------------------------------------------------------------
    // Funksjoner knyttet til "🎨 Utseende"-modalen.
    ...
    // -----------------------------------------------------------------------
    // Funksjoner knyttet til "📜 Regler"-modalen.
    ...
    // -----------------------------------------------------------------------
    // Funksjoner knyttet til redigering av spillere.
    ...
    // === 13: EVENT LISTENER ATTACHMENT (General) START ===
    // -----------------------------------------------------------------------
    // Kobler funksjoner til knapper og andre hendelser.
    ...
    btnManageAddons?.addEventListener('click', openAddonModal); // Fortsatt midlertidig

    // Dra-og-slipp for UI elementer
    draggableElements.forEach(el => { if (el) { el.addEventListener('mousedown', (e) => startDrag(e, el)); } });

    // Lukk modal ved klikk utenfor innholdet
    ...
    // --- Listeners for spesifikke Modal Knapper ---
    // Edit Player Modal
    ...
    // UI Settings Modal - legges til/fjernes dynamisk i add/removeUiModalListeners
    ...
    // --- Delelegering for knapper i spillerlistene ---
    ...
    // -----------------------------------------------------------------------
    // === 13: EVENT LISTENER ATTACHMENT (General) END ===


    // === 14: INITIAL UI RENDER & TIMER START ===
    // -----------------------------------------------------------------------
    // Kjører den første UI-oppdateringen og starter klokkene.
    ...
    // -----------------------------------------------------------------------
    // === 14: INITIAL UI RENDER & TIMER START END ===
